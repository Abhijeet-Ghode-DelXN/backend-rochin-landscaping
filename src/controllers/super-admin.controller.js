...
exports.createTenant = asyncHandler(async (req, res, next) => {
  ...
  const { name, email, subdomain, plan, status, adminPassword } = req.body;
  ...
  console.log('🏢 Creating tenant with super admin as temporary owner...');
  // Create tenant first with super admin as temporary owner
  const tenant = await Tenant.create({
    name,
    subdomain,
    owner: req.user.id, // Use super admin as temporary owner
    settings: {
  ...
  const adminUser = await User.create({
    name: `${name} Admin`,
    email,
    password: adminPassword || 'TempPassword123!', // Use provided password or fallback
    role: 'tenantAdmin',
    tenantId: tenant._id, // Now we can set the tenantId
    isEmailVerified: true
  });

  console.log('✅ Admin user created:', adminUser._id);

  console.log('🔄 Updating tenant with correct owner...');
  // Update tenant with the admin user as owner
  tenant.owner = adminUser._id;
  await tenant.save();

  console.log('✅ Tenant updated with correct owner');
